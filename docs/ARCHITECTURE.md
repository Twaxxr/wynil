...
NowSpinning uses a layered, event-driven design. The WPF composition root creates long-lived services and disposes them in reverse order. Media events produce immutable `MediaTrack` snapshots. A later bridge will version and serialize those snapshots to the WebView without exposing native objects.
